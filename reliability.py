...
# Form implementation generated from reading ui file 'reliability.ui'
#
...
class Ui_MainWindow(object):
    ...
    def calculate(self,MainWindow):
        ...
        if (bool(a2) and bool(b2)) and (not bool(c1))and (not bool(d1) and not bool(e1)):
            s = np.random.normal(a2,b2,n)
        elif (not bool(a2) and not bool(b2)) and (bool(c1))and (not bool(d1) and not bool(e1)):
            s = np.random.exponential(c1,n)
        elif (not bool(a2) and not bool(b2)) and (not bool(c1))and (bool(d1) and bool(e1)):
            s = np.random.normal(d1,e1,n)
        else:
            self.textBrowser.setPlainText('Wrong!!!!!!!!!!!')
            
        if (bool(a1) and bool(b1)) and (not bool(c2))and (not bool(d2) and not bool(e2)):
            t = np.random.normal(a1,b1,n)           
        elif (not bool(a1) and not bool(b1)) and (bool(c2))and (not bool(d2) and not bool(e2)):
            t = np.random.exponential(c2,n)           
        elif (not bool(a1) and not bool(b1)) and (not bool(c2))and (bool(d2) and bool(e2)):
        ...
        plt.savefig("F:/test.png", dpi=120)
    ...
    def drawing(self):
        ...
        self.label_13.setPixmap(QtGui.QPixmap(_fromUtf8("F:/test.png")))
